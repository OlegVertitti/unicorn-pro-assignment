...
![React + Vite](https://img.shields.io/badge/Frontend-React%20%2B%20Vite-646CFF?logo=vite&logoColor=white)
![Gemini API](https://img.shields.io/badge/LLM-Gemini%20API-4285F4?logo=googlegemini&logoColor=white)
![TypeScript](https://img.shields.io/badge/Language-TypeScript-3178C6?logo=typescript&logoColor=white)

An internal tool for a performance-marketing team: paste one or more public Google Drive links (images or short videos), get a structured analysis of the person in frame and a transcript of any speech — all powered by the Gemini API.

**🔗 Live demo:** https://unicorn-pro-assignment.olegvertitti.workers.dev
...
- [Results on the 6 provided test creatives](#results-on-the-6-provided-test-creatives)
- [How AI tools were used](#how-ai-tools-were-used)
- [Testing](#testing)
- [What I'd do next (with ~5 more hours)](#what-id-do-next-with-5-more-hours)
...
sequenceDiagram
    ...
    U->>FE: Paste 1+ Drive links, click Analyze
    FE->>W: POST /api/analyze (SSE, one per link)
    W->>D: Download file
    D-->>W: bytes + content-type
    W-->>FE: event: downloading
    alt video
        W->>G: Files API upload
        G-->>W: file_uri
        W-->>FE: event: uploading
        W->>G: poll until ACTIVE
        W-->>FE: event: processing
    end
    W->>G: generateContent (responseSchema)
    G-->>W: structured JSON
    W-->>FE: event: done { result }
    FE-->>U: parameters table + transcript + Drive preview
```
...
The entire codebase was written pair-programming with Claude Code (Sonnet 5), across the initial build and a follow-up iteration that added streaming progress, batch mode, media previews, and tests. A few things worth calling out beyond "an AI wrote the code":
...
- **The agent also drove infrastructure, not just code**: registering a workers.dev subdomain via the Cloudflare API (since `wrangler deploy` couldn't handle the corresponding interactive prompt non-interactively), setting secrets, and every deploy.
- **A second pass for polish, on request**: adding SSE progress/batch mode/previews/tests, then a dedicated cleanup pass (consistent Prettier formatting across the whole repo, a couple of small readability fixes, this README) — done as a distinct, reviewable step before anything gets pushed.
